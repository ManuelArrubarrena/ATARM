...
int Serre; int magic; double accBala1A; double accBala1B; double VelaOpen; double VelaOpen2; double VelaOpen3; double accountBalance; double Adios = 0.01; double PricePromStdV = 0; double PriceProm2StdV = 0; double PriceProm3StdV = 0; double PriceProm4StdV = 0; double PriceProm5StdV = 0;
...
bool CoInVel = true; bool VeInVel = true; bool CoAl38 = true; bool VeAl38 = true; bool PrintTV = false; bool PrintTC = false; bool calent = false;
...
double prediV = 0; double PrediV = 0; double prediV1 = 0; double PrediV1 = 0; double prediPrice = 0; double prediPrice1 = 0; double prediRf = 0;
double prediCubA = 0; double prediCubB = 0; double MatrixCub[1][4]; double Conex = 0;
...
//--- create timer
   ...
   RExecute(Serre,"Neuronss = 30"); RExecute(Serre,"LEarNr = 0.0015"); RExecute(Serre,"maxiter = 3500");
   ...
   RExecute(Serre,"Rsnns1HV = jordan(equisV, yeV, size = c(Neuronss), maxit = maxiter, initFunc = 'JE_Weights', initFuncParams = c(1, -1, 0.3, 1, 0.5), learnFunc = 'JE_BP', learnFuncParams = c(LEarNr), updateFunc = 'JE_Order', updateFuncParams = c(0), shufflePatterns = FALSE, linOut = TRUE, inputsTest = NULL, targetsTest = NULL)");
   Print("AUDJPY red lista carnal");
   RExecute(Serre,"library(Cubist)");
   RExecute(Serre,"dataCb = read.csv('C:/Users/Administrator/Desktop/AudJpy&Cub2vel1H.csv', header = TRUE)");
   RExecute(Serre,"Xcub = dataCb[ , -5]");
   RExecute(Serre,"Ycub = dataCb[ , 5]");
   RExecute(Serre,"CubA1 = cubist(x = Xcub, y = Ycub, committees = 30)");
   Print("AUDJPY cubistA listo carnal");
   MatrixCub[0][0] = 0.057; MatrixCub[0][1] = 0.087; MatrixCub[0][2] = 0.4; MatrixCub[0][3] = 0.015;
   ...
   RExecute(Serre, "predCubA1 = predict(CubA1, MatrixCub1H, neighbors = 4)");
   Conex = RGetDouble(Serre, "ifelse(predCubA1 > 0.5, '1', '-1')");
   Print("Prueba conex ", Conex);
...
void checkBalance(){
   ...
   if(hour < 1.5 || hour > 22){
      ...
      Print("AccBalaA1 ", accBala1A);
   }
}

void FiveTeMin(bool canContinue){
   double macd; 
   ...
   double HIgh = iHigh(Symbol(),60,0); double LOw = iLow(Symbol(),60,0); double HIgh1 = iHigh(Symbol(),60,1); double LOw1 = iLow(Symbol(),60,1); double HIgh2 = iHigh(Symbol(),60,2); double LOw2 = iLow(Symbol(),60,2); double HIgh3 = iHigh(Symbol(),60,3); double LOw3 = iLow(Symbol(),60,3);
   ...
   double volumeA = accBala1A;
   volumeA = NormalizeDouble(volumeA, 1);
   double VelaGrande = 2000; double VelaCanal = 2000;
   ...
   if(canContinue)
      if(resultCheckOrders){
         ...
         //Condicion Velota
         if((HIgh2 - LOw2) >= VelaGrande || (HIgh1 - LOw1) >= VelaGrande || (LOw - HIgh) >= VelaGrande){Velota = false;} else{Velota = true;}
         //Condicion GAP Kids
         ...
         //Condicion PATOTAS
         if(OPen1 >= CLose1){
            if((CLose1 - LOw1) >= Cuerpo1 || (HIgh1 - OPen1) >= Cuerpo1){Patitas1 = false;} else{Patitas1 = true;}
         }else{if((OPen1 - LOw1) >= Cuerpo1 || (HIgh1 - CLose1) >= Cuerpo1){Patitas1 = false;} else{Patitas1 = true;}
         }if(OPen2 >= CLose2){
            if((CLose2 - LOw2) >= Cuerpo2 || (HIgh2 - OPen2) >= Cuerpo2){Patitas2 = false;} else{Patitas2 = true;}
         }else{if((OPen2 - LOw2) >= Cuerpo2 || (HIgh2 - CLose2) >= Cuerpo2){Patitas2 = false;} else{Patitas2 = true;}
         ...
         //Calentamiento
         if(OpenHour > 1.5 && OpenHour < 1.583){
            calent = false;
            if(VelaOpen3 != Uopen){
               RAssignMatrix(Serre, "MatrixV1H", MatrixV, 1, 5);
               RExecute(Serre, "colnames(MatrixV1H)<-c('MM','Volumen1Std', 'macdStd', 'ind1Std', 'PorFiboStd')");
               RExecute(Serre, "MatrixV1H = data.frame(MatrixV1H)");
               RExecute(Serre, "predV1H = predict(Rsnns1HV, MatrixV1H)");
               prediV = RGetDouble(Serre, "predV1H");
               VelaOpen3 = Uopen;
               Print("not llet");
            } 
         }else{calent = true;}
         if(calent == true){
            ...
            if(VelaOpen != OPen){
               ...
               RExecute(Serre, "TiempoA15 = matrix(1:4)");
               RExecute(Serre, "RlinA1 = lm(DataA1 ~ TiempoA15)");
               ...
               RExecute(Serre, "TiempoB15 = matrix(1:4)");
               RExecute(Serre, "RlinB1 = lm(DataB1 ~ TiempoB15)");
               ...
               RExecute(Serre, "MatrixCub1H = data.frame(MatrixCub1H)");
               RExecute(Serre, "predCubA1 = predict(CubA1, MatrixCub1H, neighbors = 4)");
               prediCubA = RGetDouble(Serre, "ifelse(predCubA1 > 0.5, '1', '-1')");
               Print("PredA : ", prediCubA);
            }
            if(VelaOpen2 != MOPen){
               ...
               RExecute(Serre, "colnames(MatrixV1H)<-c('MM','Volumen1Std', 'macdStd', 'ind1Std', 'PorFiboStd')");
               RExecute(Serre, "MatrixV1H = data.frame(MatrixV1H)");
               RExecute(Serre, "prediV1H = predict(Rsnns1HV, MatrixV1H)");
               PrediV = RGetDouble(Serre, "prediV1H");
            }           
            //VENTA 
            ...
            //Vende al 50% de vela
               if(prediCubA < 0){
                  ...
                  int ticket = OrderSend(Symbol(),OP_SELL,volumeA,price,3,stoploss,takeprofit,"My order SELL",magic,0,clrGreen);
                  if(ticket<0){
                    ...
                    Print("Pend5 ", Pend25, " Pend10 ", Pend10, " PorFiboStdV ", PorFiboStdV, " DifMax ", DifCar, " DifMin ", DifCab, " DifMax2 ", (DifCar*DifCar), " DifMin2 ", (DifCab*DifCab), " Volumen1StdV ", Volumen1StdV);
                    VeAl38 = false; VelaOpen = OPen; 
                  ...
                  takeprofit = NormalizeDouble(takeprofit, Digits);
                  int ticket = OrderSend(Symbol(),OP_BUY,volumeA,price,3,stoploss,takeprofit,"My order BUY ",magic,0,clrGreen);
                  if(ticket<0){
                    Print("OrderSend failed with error #",GetLastError());
                  }else{
                    Print("CompraA");
